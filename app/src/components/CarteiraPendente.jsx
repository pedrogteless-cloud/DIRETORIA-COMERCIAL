...
// Lista o que cada representante ainda tem pra entregar (pedido lançado,
// não faturado), por fábrica — separado do ranking de comissão paga, que
// é outra métrica (mede a fábrica, não o vendedor; ver CONTEXT.md).
export default function CarteiraPendente({ rows, dateColch, dateColtim }) {
  ...
  return (
    <div className="rounded-xl border border-border overflow-hidden bg-panel">
      ...
      {rows.length === 0 && <div className="px-4.5 py-6 text-center text-muted text-sm">Nenhuma carteira pendente registrada ainda.</div>}
      ...
      {rows.length > 0 && (
        <div className="grid grid-cols-[1fr_130px_130px_130px] items-center gap-3.5 px-4.5 py-3 bg-panel2 text-xs text-muted">
          <div>
            Eusébio: carteira de {fmtDateShort(dateColch)} · Timon: carteira de {fmtDateShort(dateColtim)}
          </div>
          <div className="text-right font-mono">{fmtBRL(totalColch)}</div>
          <div className="text-right font-mono">{fmtBRL(totalColtim)}</div>
          <div className="text-right font-mono font-semibold text-white">{fmtBRL(totalColch + totalColtim)}</div>
        </div>
      )}
    </div>
  )
